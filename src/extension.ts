...
import { loadConfig, saveDeviceToken, clearDeviceToken, type PluginConfig } from "./config"
...
// applyConfig (re)reads the shared config and brings the runtime in line with it:
// it tears down any prior serving state, then either serves + patches every
// supported surface (token present, not opted out) or leaves every bundle pristine.
// It is called at boot AND after a sign-in / sign-out command, so a token entered
// in-editor takes effect immediately without reloading the window.
async function applyConfig(): Promise<void> {
  const cfg = loadConfig(envRecord())
  ...
  for (const surface of surfaces.values()) {
    ...
    try {
      await serveAndPatch(extContext, client, surface)
      scheduleRotation(extContext, client, cfg, surface)
    } catch (e) {
...
function registerCommands(context: vscode.ExtensionContext): void {
  ...
  cmd("vibeperks.signIn", async () => {
    // If a device token is already saved (shared ~/.vibeperks/config.json), reuse it
    // instead of forcing a fresh login; only prompt for a new token when the user
    // explicitly chooses to replace it.
    const current = loadConfig(envRecord())
    if (current.deviceToken !== "") {
      const choice = await vscode.window.showInformationMessage(
        "VibePerks: already signed in on this machine.",
        "Replace token",
      )
  ...
  cmd("vibeperks.signOut", async () => {
    ...
    clearDeviceToken(envRecord())
    await applyConfig()
    await vscode.window.showInformationMessage("VibePerks: signed out.")
  })
